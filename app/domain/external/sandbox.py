...
class Sandbox(Protocol):
    ...
    async def view_shell(self, session_id: str, console: bool = False) -> ToolResult:
        """根据传递的会话id+是否返回控制台记录 获取shell结果"""
        ...

    async def wait_for_process(
        self, session_id: str, seconds: int | None = None
    ) -> ToolResult:
        """根据传递的会话id+秒数 等待程序执行"""
        ...
    ...
    async def file_write(
        ...
        """根据传递的文件路径+写入内容+追加模式+前后内容新行+超级权限 写入对应的文件"""
        ...
    ...
    async def file_read(
        ...
        """根据传递的文件路径+起点终点行数+超级权限 读取对应的文件内容"""
        ...
